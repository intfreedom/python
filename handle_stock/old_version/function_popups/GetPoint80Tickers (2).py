...
def GetLowest(ticket, startT='2018-06-01',Distance = 30, rateDistance=1.08):
    dateToday = datetime.datetime.today().strftime('%Y-%m-%d')
    lowVaule = ''
...
TickersFileName = 'AllTickets'+dateToday
...
fileSavePoint80Name = '00000000-10101000-Point80-'+dateToday    # 用于记录历史分析；
saveCode(fileSavePoint80Name, Point80)
fileSavePoint80Fixed = '10101000-Point80'    # 用于读取，每次读取一个固定文件；
fileaddress = r'./Data/'
if os.path.exists(fileaddress+'10101000-Point80'+'.csv'):    #其实不必，saveCode的这种写法直接覆盖原文件
    os.remove(fileaddress+'10101000-Point80'+'.csv')
saveCode(fileSavePoint80Fixed, Point80)    # 'w' 只能写 覆盖整个文件 不存在则创建 
# 这两者的区别就是控制数量；看看差异大小；
...
if os.path.exists(fileaddress+'10101000-Point90'+'.csv'):    #其实不必，saveCode的这种写法直接覆盖原文件
    os.remove(fileaddress+'10101000-Point90'+'.csv')
