...
for code_number in column[0:3]:
    try:
        history = ts.get_hist_data(code_number)
        print("---", type(history), "---")
        ...
        if filter_code_new(code_number, len(open_value)):
            code_point = one_two_three(code_number)
            print("### ", code_point, " ###")

            if code_point[0] != '' and get_lowest(code_number) != '':
                one_day_lowest.append(code_number)

            if code_point[1] != '' and get_lowest(code_number) != '':
